...
class NotebookApiClient {
  ...
  Future<PaginatedPrayerRequests> fetchNotebookRequestsAt(
      CursorPagination pagination, int? groupId, int? contactId,
      {int? eventId, int? collectionId, int? relatedContactId}) async {
    var input = {
    ...
    if (response.statusCode != 200) {
      throw Exception(
          "Error getting notebook: ${response.statusCode} - ${response.body}");
    }

    return PaginatedPrayerRequests.fromJson(
        jsonDecode(utf8.decode(response.bodyBytes)));
  }

  Future<PaginatedCollections> fetchNotebookCollectionsAt(
      CursorPagination pagination, int groupId) async {
    ...
    return PaginatedCollections.fromJson(
        jsonDecode(utf8.decode(response.bodyBytes)));
  }
}
